...
This guide is for developers joining the project. The current implementation includes Phase 1 authentication and the complete, verified Phase 2 Applicant Module. Phase 3 and later modules are not implemented.
...
The schema creates the `passport_automation` database and the current tables for users, applications, documents, and payments.
...
- `backend/models/`: SQLAlchemy models for users, applications, documents, and payments.
- `backend/routes/`: authentication and applicant API blueprints.
...
- Do not start Phase 3 until the project owner approves it.
